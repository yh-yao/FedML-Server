...
from FedML.fedml_api.distributed.fedopt.FedOptAggregator import FedOptAggregator
from FedML.fedml_api.distributed.fedopt.FedOptServerManager import FedOptServerManager
from FedML.fedml_api.distributed.fedopt.MyModelTrainer import MyModelTrainer
...
def load_data(args, dataset_name):
    if dataset_name == "mnist":
        ...
        class_num = load_partition_data_shakespeare(args.batch_size,
                                                    train_path="./../FedML/data/shakespeare/train",
                                                    test_path="./../FedML/data/shakespeare/test")
        args.client_num_in_total = client_num
    else:
...
if __name__ == '__main__':
    ...
    wandb.init(
        ...
        name="mobile(mqtt)" + str(args.partition_method) + "r" + str(args.comm_round) + "-e" + str(
            args.epochs) + "-lr" + str(
            args.lr) + "-time" + str(time.time()) + "-server fedopt",
        config=args
    )
    ...
    server_manager = FedOptServerManager(args,
                                         aggregator,
                                         rank=0,
                                         size=size,
                                         backend="MQTT")
